...
aws s3api put-object-tagging --bucket ${S3_BUCKET_NAME} --key ${CAPTAIN_DOMAIN}/${BACKUP_PREFIX}/${date}/vault_${datetime}.snap --tagging "TagSet=[{Key=datetime,Value=${datetime}},{Key=level,Value=primary}]"
